...
describe('SignalK Client listAvailablePaths - Live Integration', () => {
  ...
  test('should discover available paths via HTTP API with fallback behavior', async () => {
    ...
    const timestampAge = Date.now() - new Date(availablePaths.timestamp).getTime();
    ...
    console.log(`Path discovery: ${availablePaths.count} paths, ${responseTime}ms response time`);
  ...
  test('should handle HTTP failure gracefully with WebSocket fallback', async () => {
    ...
    if (availablePaths.error) {
      console.log('HTTP failed, WebSocket fallback used:', availablePaths.error);
    ...
    console.log(`Fallback test: ${availablePaths.count} paths, ${responseTime}ms response time`);
  ...
  test('should maintain path consistency over time', async () => {
    ...
    console.log(`Path consistency: ${paths1.count} → ${paths2.count} (${paths2.count - paths1.count} added)`);
  });
  ...
  test('should handle empty paths response gracefully', async () => {
    ...
    console.log(`Empty paths test: ${availablePaths.count} paths, ${responseTime}ms response time`);
  });
});