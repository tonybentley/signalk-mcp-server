...
describe('SignalK Client getConnectionStatus - Live Integration', () => {
  ...
  test('should return valid connection status structure when connected', () => {
    ...
    console.log(
      `Server: ${status.hostname}:${status.port} (TLS: ${status.useTLS})`,
    );
    console.log(
      `Data counts - Paths: ${status.pathCount}, AIS: ${status.aisTargetCount}, Alarms: ${status.activeAlarmCount}`,
    );
  });
  ...
  test('should reflect real-time changes in data counts over time', async () => {
    ...
    expect(status2.aisTargetCount).toBeGreaterThanOrEqual(
      status1.aisTargetCount,
    );
    expect(status2.activeAlarmCount).toBeGreaterThanOrEqual(
      status1.activeAlarmCount,
    );
    ...
    console.log(
      `Status evolution: Paths ${status1.pathCount}→${status2.pathCount}, AIS ${status1.aisTargetCount}→${status2.aisTargetCount}, Alarms ${status1.activeAlarmCount}→${status2.activeAlarmCount}`,
    );
  ...
  test('should maintain status consistency before and after disconnect', async () => {
    ...
    expect(disconnectedStatus.aisTargetCount).toBe(
      connectedCounts.aisTargetCount,
    );
    expect(disconnectedStatus.activeAlarmCount).toBe(
      connectedCounts.activeAlarmCount,
    );

    // Timestamp should be recent (status is freshly generated)
    const timestampAge =
      Date.now() - new Date(disconnectedStatus.timestamp).getTime();
    ...
    try {
      ...
      console.log(
        `Connection lifecycle: Connected→Disconnected→(Reconnect failed in test environment)`,
      );
      console.warn(
        'Reconnection failed (acceptable in test environment):',
        (error as Error).message,
      );
    }

    console.log(
      `Data preserved through disconnect: ${connectedCounts.pathCount} paths, ${connectedCounts.aisTargetCount} AIS, ${connectedCounts.activeAlarmCount} alarms`,
    );
