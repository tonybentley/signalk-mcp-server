...
describe('SignalK Client getVesselState - Live Integration', () => {
  ...
  test('should connect to live server and return valid vessel state structure', async () => {
    ...
    expect(vesselState.context).toBe(process.env.SIGNALK_CONTEXT || 'vessels.self');
    ...
    console.log(`Vessel state contains ${Object.keys(vesselState.data).length} data paths`);
  });
  ...
  test('should return consistent dynamic paths and maintain data integrity over time', async () => {
    ...
    console.log(`Dynamic paths: ${paths1.length} → ${paths2.length} (${paths2.length - paths1.length} added)`);
    console.log(`Available paths on server: ${availablePaths.paths.length}`);
    console.log(`Data coverage: ${(paths2.length / Math.max(availablePaths.paths.length, 1) * 100).toFixed(1)}%`);
