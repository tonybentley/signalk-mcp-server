...
describe('SignalK Client getAISTargets - Live Integration', () => {
  ...
  test('should detect and filter AIS targets with live vessel traffic data', async () => {
    ...
    if (aisTargets2.count > 0) {
      ...
      const targetsWithPosition = aisTargets2.targets.filter(t => t['navigation.position']);
    ...
    if (aisTargets2.count > 0) {
      const avgAge = aisTargets2.targets.reduce((sum, target) => {
  ...
  test('should handle empty AIS response gracefully', async () => {
    ...
    console.log(`Empty AIS response test: ${aisTargets.count} targets, ${responseTime}ms response time`);
  });
});