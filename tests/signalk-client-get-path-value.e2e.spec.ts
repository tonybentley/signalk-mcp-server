...
describe('SignalK Client getPathValue - Live Integration', () => {
        ...
        switch (category) {
          case 'navigation':
            if (testPath.includes('position')) {
              ...
              if (typeof pathResult.data.value === 'object' && pathResult.data.value) {
            ...
            } else if (testPath.includes('speed') || testPath.includes('course') || testPath.includes('heading')) {
          ...
          case 'environment':
            if (testPath.includes('wind') || testPath.includes('depth') || testPath.includes('temperature')) {
          ...
          case 'electrical':
            if (testPath.includes('voltage') || testPath.includes('current') || testPath.includes('power')) {
  ...
  test('should maintain data consistency and handle real-time updates', async () => {
    ...
    const dynamicPaths = availablePaths.filter(path => 
